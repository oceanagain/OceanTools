:: Made by Ocean
:: Discord: https://discord.gg/un2p2yArJ4
 
@echo off
title OCEAN V1
color 4

:: (OCEAN) Check for Admin Privileges.
fltmc >nul 2>&1
if not %errorlevel% == 0 (
    powershell -Command "Write-Host 'OCEAN is required to be run as *Administrator.*' -ForegroundColor White -BackgroundColor Red" 
...
:: (OCEAN) Check for Windows Defender.
...
:: (OCEAN) Check if TrustedInstaller is disabled, for Nsudo.
...
:: (OCEAN) Downloading OceanTools at Start, Includes NSudo, Orca and Amd.
@echo off
REM Set the download URL and output file name
set DOWNLOAD_URL=https://github.com/oceanagain/OceanTools/raw/main/Downloads/OceanTools.zip
set ZIP_FILE=OceanTools.zip
set EXTRACT_DIR=OceanTools
...
REM Extract the ZIP file using tar (available in Windows 10 and later)
...
echo.                                                                
echo.                                                                
echo.                              .g8""8q.     .g8"""bgd `7MM"""YMM        db      `7MN.   `7MF'
echo.                            .dP'    `YM. .dP'     `M   MM    `7       ;MM:       MMN.    M  
echo.                            dM'      `MM dM'       `   MM   d        ,V^MM.      M YMb   M  
echo.                            MM        MM MM            MMmmMM       ,M  `MM      M  `MN. M  
echo.                            MM.      ,MP MM.           MM   Y  ,    AbmmmqMA     M   `MM.M  
echo.                            `Mb.    ,dP' `Mb.     ,'   MM     ,M   A'     VML    M     YMM  
echo.                              `"bmmd"'     `"bmmmd'  .JMMmmmmMMM .AMA.   .AMMA..JML.    YM  
echo.                                                                
echo.                                                               
echo.
echo. 
echo.                                  ╔════════════════════════════════════════════════════╗
echo.                                  ║              Version 1 - By Griffin                ║
echo.                                  ╚════════════════════════════════════════════════════╝
echo.
echo.
echo. ╔═════════╗                                                                        
...
:Yes
echo You chose Yes, running the script...
start "" "C:\Users\%USERNAME%\Desktop\OceanTools\Oneclicktools\Activate Windows.bat"
goto End
...
:: Ask the user if they want to debloat Nvidia drivers
set /p user_input="Would you like to debloat Nvidia Drivers? (yes/no): "

:: Convert the input to lowercase
for %%A in (%user_input%) do set input=%%A
set input=%input:~0,1%

:: Check if the user entered 'y' for Yes
if /i "%input%"=="y" (
    echo You chose to debloat Nvidia Drivers.
    echo Opening the OceanTools > DDU folder and launching Display Driver Uninstaller...
    
    :: Open the "OceanTools" folder
    start explorer "C:\Users\%USERNAME%\Desktop\OceanTools\Oneclicktools\DDU"
    
    :: Wait for the folder to open before launching the program
    timeout /t 3 /nobreak >nul
    
    :: Launch "Display Driver Uninstaller.exe"
    start "" "C:\Users\%USERNAME%\Desktop\OceanTools\Oneclicktools\DDU\Display Driver Uninstaller.exe"
) else (
    echo You chose not to debloat Nvidia Drivers.
)
...
cls
color 9
echo (OCEAN) Enabling Hardware Accelerated GPU Scheduling...
...
cls
color D
echo (OCEAN) Disabling Transparency Effects...
...
cls
color 9
echo (OCEAN) Running OOshutup10 with imported optimized settings...
...
set "cfgFileName=OCEANOOshutup10.cfg"
mkdir "C:\Users\%USERNAME%\Desktop\OceanTools\Oneclicktools\OOShutup10" >nul 2>&1
...
:Dis
echo Going to Ocean's Tweaks Discord...
start "" "ddd"
timeout 2 > nul
goto :Extra
